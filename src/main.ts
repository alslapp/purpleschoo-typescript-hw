const makeOrdinal = (words: string): string => {
	console.log(`fn makeOrdinal, words: ${words}`);
	return "zero";
}

const isFinite = (num: number): boolean => {
	console.log(`fn isFinite, num: ${num}`);
	return true;
}

const isSafeNumber = (num: number): boolean => {
	console.log(`fn isSafeNumber, num: ${num}`);
	return true;
}
...
function generateWords(number: number, ...rest: any[]): string {
	let remainder = 0;
